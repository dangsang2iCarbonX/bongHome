...
public class SummaryLayout extends View {
    ...
    private static final long DONE_DUR = 800;
    ...
    public SummaryLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean isClosing() {
        return mAniStatus == AnimatorStatus.DONE;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.i(TAG, "onDraw: status = " + mAniStatus.toString());

        switch (mAniStatus) {
            case PULL_DOWN: //下拉
                canvas.drawRect(0, 0, mWidth, mHeight, mBackPaint);
                break;
            case REL_DRAG://release drag 释放刷新
            case DRAG_DOWN: //拖拽
                drawDrag(canvas);
                break;
            case SPRING_UP: //从水平面弹起弧线
                drawSpring(canvas, getSpringDelta());
                invalidate();
                break;
            case POP_BALL: //从弧线里分离出球
                drawPopBall(canvas);
                invalidate();
                break;
            case OUTER_CIR: //弧线坠落
